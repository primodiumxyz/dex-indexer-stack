 ...
 *
 * Note: This is a very expensive operation, designed for internal benchmarking and testing.
 *
 * @returns An array of tokens (`mint` column)
 */
...
/**
 * Queries the top tokens sorted by volume during the last 30 minutes.
 *
...
/**
 * Queries the metadata and computed stats for a single token.
 *
 * @param token - The mint address of the token
 * @returns The metadata and computed stats for the token
 * @see {@link GetTopTokensByVolumeQuery} for the return type
 */
...
/**
 * Queries the metadata and computed stats for multiple tokens.
 *
...
/**
 * Queries the trade history for a single token since a given time.
 *
 * @param token - The mint address of the token
 * @param since (optional, default: "now()") - The start time of the query
 * @returns An array of trade history entries:
 *
 *   - `token_price_usd` - The new price of the token in USD
...
/**
 * Queries the candle history for a single token since a given time.
 *
 * @param token - The mint address of the token
 * @param since (optional, default: "now()") - The start time of the query
 ...
 *   - `has_trades` - Whether the token had any trades in the bucket
 */
